...
class Eventhub:
  ...
  async def __read(self):
    try:
      ...
      if rpcId in self._rpc_awaitables:
        ...
        asyncio.create_task(self._subscription_callbacks[rpcId](resp['result']['topic'], resp['result']['message']))
    except:
      pass
  ...
  async def disconnect(self):
    try:
      await asyncio.wait(self.__rpc_request_wait_for_response("DISCONNECT", {}), 1)
  ...
  async def subscribe(self, topic, callback):
    resp = await self.__rpc_request_wait_for_response("SUBSCRIBE", {
      "topic": topic
    })
  ...
  async def publish(self, topic, message):
    return await self.__rpc_request_wait_for_response("PUBLISH", {
      "topic": topic,
      "message": message
    })

  async def set(self, key, value):
    return await self.__rpc_request_wait_for_response("SET", {
      "key": key,
      "value": value
    })

  async def get(self, key):
    res = await self.__rpc_request_wait_for_response("GET", {
      "key": key
    })
  ...
  async def delete(self, key):
    return await self.__rpc_request_wait_for_response("DEL", {
      "key": key
    })